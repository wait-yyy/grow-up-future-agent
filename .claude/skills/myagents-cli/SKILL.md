...
name: myagents-cli
description: >-
  你正在 MoneyAI-Agents 这款 AI 产品里运行——MoneyAI-Agents 自带一套"产品能力"（定时任务、任务中心、想法收集、MCP 工具接入、
  模型 Provider、IM Bot 渠道、社区插件、Skills 安装、MoneyAI-Agents Cloud Space、Generative UI Widget 等），全部通过内置 `myagents` CLI 暴露给你。
  当用户的需求**落在 MoneyAI-Agents 产品能力的射程内**，就加载并使用这个 skill，用 CLI 主动帮用户把事情做掉，
  而不是让用户去 GUI 点击。
  ...
  即使用户没说"用 MoneyAI-Agents 做"几个字，只要意图能映射到上述能力之一，就该走这个 skill。
  反向边界：纯业务任务（写代码、查资料、读文件）不归这里；用户自己会话里给 AI 排任务用 im-cron MCP，不是这里。
author: MoneyAI-Agents
---

# myagents-cli — MoneyAI-Agents 产品能力的 CLI 入口

你正运行在 MoneyAI-Agents 产品内。MoneyAI-Agents 不只是一个 chat UI，它是一套带状态的 Agent 平台：定时任务、任务中心、IM Bot、MCP、Provider、插件、Skill、Cloud Space、Widget——这些都是产品能力，由内置 `myagents` CLI 一站暴露给你。

**这个 skill 不只是"管理工具"，它是 MoneyAI-Agents 产品能力的执行入口**。用户表达的需求只要能映射到产品能力，就该用 CLI 主动帮用户做掉，而不是给用户一堆操作步骤让他自己去 Settings 点。这份文档列出全部能力以及"什么时候应该用哪条命令"。
...
这是 MoneyAI-Agents 内置官方 CLI 工具，不属于 MCP，也不属于用户注册 `tool` 实验功能。它用于在当前会话启用“图片理解”工具时，让不支持多模态的主模型把本地工作区图片交给用户在「设置 → 工具箱」里配置好的读图模型分析。
...
**约束：**
- 只接受当前 MoneyAI-Agents 工作区内的本地图片路径；不要传 URL。
...
- 「@oai/artifact-tool 我从终端能调用、MoneyAI-Agents 里就不行」/「Codex MCP 在 MoneyAI-Agents 里看不到」/「Codex 是不是用错代理了」→ `runtime diagnose codex`。它 spawn 一个临时 codex app-server，跑 `getAuthStatus` / `experimentalFeature/list` / `mcpServerStatus/list` / `app/list` 四个 RPC，把 Codex 自己看到的状态原样吐出来，省得猜。effectiveEnv 节里能看到 MoneyAI-Agents 注入的代理是不是真到了子进程，feature flag 是不是真生效。
...
- 用户在 `~/.claude/skills/` 自己塞了东西 MoneyAI-Agents 看不见 → `skill sync`
...
### MoneyAI-Agents Cloud Space（space）
...
**何时用：**
- 你是在 Cloud Space 派发出来的本地 Task 里工作，Task.md 说“你收到了一个 MoneyAI-Agents Space Issue 通知” → 先 `myagents issue <issueId> --json` 拉完整 Issue；旧环境可用 `myagents space issue get <issueId> --json`。
...
- `--file` 必须是绝对路径，且路径白名单：必须落在 workspace / `/tmp` / MoneyAI-Agents scratch 目录之一——这是为了防 prompt injection 把 `~/.ssh/id_rsa` 之类发给聊天对方
...
# 默认: 目标 turn 结束后,MoneyAI-Agents 会把结果推回当前 session
...
MoneyAI-Agents 基于 Claude Agent SDK，原生协议是 Anthropic Messages API。接入第三方 API 时：
...
大多数支持 Anthropic 协议的服务商，会在文档里以「接入 Claude Code」的形式呈现——MoneyAI-Agents 和 Claude Code 共享 SDK，所以 Claude Code 的接入方式就是我们最原生的接入方式。
...
| Claude Code 环境变量 | MoneyAI-Agents CLI |
|---------------------|------------|
| `ANTHROPIC_BASE_URL` | `--base-url` |
